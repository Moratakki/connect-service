...
async def volunteer_profile_faculty(message: types.Message, state=FSMContext):
    ...
    await message.answer('Ваша анкета принята в обработку.')



def register_volunteer_handlers(dp: Dispatcher):
    dp.register_message_handler(volunteer_profile_init, commands=['Волонтёр'], state=None)
    dp.register_message_handler(cancel_registration, state="*", commands="Отмена")
    ...
    dp.register_message_handler(resume_registration, commands="Регистрация", state=None)
